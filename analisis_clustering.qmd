...
En este documento vamos a realizar un análisis con Clustering de las variables cuantitativas de nuestro dataset Spotify 2023. Lo haremos de dos formas: a partir de las 50 canciones más escuchadas, viendo si los métodos de clustering las agrupan según su género musical; y a partir de las canciones de los 5 artistas más escuchados, comprobando si los métodos las agrupan según su artista.
...
Cada análisis de clustering lo dividiremos en tres partes, donde realizaremos tres técnicas aprendidas en clase: el método de *k-means*, el método *k-medoids* y el método de clustering jerárquico aglomerativo, que a su vez realizaremos también tres veces con los siguientes métodos de distancias entre clusters: enlace completo, enlace medio y Ward.
...
Primero, representaremos estas 50 canciones en el plano formado por las dos primeras componentes principales con el objetivo de ver inicialmente la distribución de estas canciones. Esta representación se hará únicamente con las variables porcentuales, por dos razones: estas variables están en la misma unidad de medida y queremos tener en cuenta la variabilidad de estas variables, por lo tanto considerando solo las porcentuales no es necesario tipificar (únicamente centrar); y, por otra parte, al ser estas observaciones las canciones más escuchadas, tiene más sentido representar estas canciones según las propiedades de estas.

```{r var porcentajes top50}
...
fviz_pca_ind(sp_perc_cen_acp50, col.ind = sp_perc_cen50$genero, repel = TRUE,
             label = FALSE, title = "Gráfico de las top50 canciones con ACP",
...
Para el método de *k-means* hay que elegir previamente el número de clusters que deseamos. En nuestro caso, como el objetivo es agrupar por género, decidimos que el número de clusters sea 6. Los centros dejaremos que se seleccionen de manera aleatoria.
