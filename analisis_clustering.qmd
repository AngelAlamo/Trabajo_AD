...
Debido al gran número de observaciones que tenemos, en este apartado haremos un breve Análisis de Componentes Principales y el estudio de clustering de nuestros datos tomando las 50 canciones más escuchadas hasta verano de 2023 en Spotify. A partir de estos clusterings, obtenidos a partir de las variables cuantitativas porcentuales, obtenemos una buena distinción de las observaciones. Nuestros grupos de referencia será el género musical de la canción.


Por lo tanto, tomamos los datos de las 50 canciones más escuchadas y después (con ayuda de internet) creamos una nueva variable que nos da el género musical de estas 50 canciones:
...
Nuestra tabla de datos es la siguiente (con los datos ya centrados):
...
fviz_eig(sp_perc_cen_acp50, addlabels = TRUE, ylim = c(0,100)) +
  labs(title = "Varianza explicada por cada valor propio", x = "Porcentaje de la varianza explicada", y = "Dimensiones") +
  theme(plot.title = element_text(size = 15))

```

A partir de este gráfico podemmos ver que con dos componentes principales es suficiente para la representación de los datos. Veamos los puntos en el plano:
...
autoplot(sp_perc_cen_acp50, data = sp_perc_cen50, #colour = 'genero', #no entiendo pk no me pinta por colores y asi tener las agrupaciones autenticas
         ...
         loadings.label = TRUE, loadings.label.size = 3) +
  theme_bw() +
  labs(title = "Gráfico de las top50 canciones con ACP", subtitle = "Variables porcentuales") +
  theme(plot.title = element_text(size = 15), plot.subtitle = element_text(size = 12))
  
```

A partir de esta representación se podrían deducir tres grupos, diferenciados a partir de los valores que toman en la primera componente principal, de manera que estas canciones se separarían según el nivel de acústico, la energía, o el balance de estas variables. Veamos esto con más detalle a partir de los métodos de cluster.



Lo dividiremos en tres partes, donde realizaremos tres técnicas aprendidas en clase: el método *k-means*, el método *k-medoids* y el método de clustering jerárquico aglomerativo.
...
#### No hace falta centrar ni escalar (creo)
...
Las agrupaciones que se obtienen
