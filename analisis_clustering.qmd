...
Utilizaremos este método con tres tipos de distancia:completa, media y Ward. Todos ellos utilizarán como matriz de distancia la obtenida aplicando la distanica euclidea:
...
# Dendograma
dend_complet_50 = hclust(d = mat_dist50, method = "complete")

```

```{r dend_completo}

fviz_dend(x = dend_complet_50, k = 6, cex = 0.6)

```

```{r interpretacion enlace simple}
...
cor(x = mat_dist50, cophenetic(dend_complet_50))
...
fviz_cluster(object = list(data = spotify_perc50, cluster = cutree(dend_complet_50, k = 6)),
 ellipse.type = "convex",
 repel = TRUE,
...
# Dendograma
dend_med_50 = hclust(d = mat_dist50, method = "average")

```

```{r dend_compl}

fviz_dend(x = dend_med_50, k = 6, cex = 0.6)

```

```{r cor_compl50}
print("Coeficiente de correlación entre las distancias cophenetic")
cor(x = mat_dist50, cophenetic(dend_med_50))
...
fviz_cluster(object = list(data = spotify_perc50, cluster = cutree(dend_med_50, k = 6)),
 ellipse.type = "convex",
 repel = TRUE,
 show.clust.cent = FALSE) +
theme_bw()
...
Recordemos que se considera una buena representación si $> 0.75$. En este caso, la representación no es lo suficientemente buena y tendremos que basarnos en las otras distancias para sacar conclusiones. A continuación mostramos los clusters obtenidos en el plano.
...
Repetimos el proceso anterior, consdierando ahora la distancia media entre clusters.
