...
Una vez vistos el pequeño resumen del dataset y las descripciones de cada variable, procedamos a estudiar las secciones consideradas, no sin antes responder a dos preguntas de psoible interés para el lector, que son determinar las 10 canciones y los 10 artistas más escuchados en Spotify 2023:
...
```{r correlacion entre variables cuantitativas, fig.width = 15, fig.height = 8}
spotify2 %>%
  select(where(is.numeric)) %>%
  na.omit(.) %>% 
  cor(.) %>% 
  corPlot(.,
       cex = 1 , main = "Correlación Entre Variables Cuantitativas"
       )
```


Ahora empezamos a hacer el estudio para dar respuesta a nuestros puntos de interés.
...
```{r matriz dispersion, fig.height=20, fig.width=20}
matriz_dispersion <- spotify2 %>% 
  select(19:23) %>% 
  ggpairs(aes(color = spotify2$mode))
...
```{r matriz dispersion top100, fig.height=20, fig.width=20}
matriz_dispersion_100 <- top100canciones %>% 
  select(19:23) %>% 
  ggpairs(aes(color = top100canciones$mode))
