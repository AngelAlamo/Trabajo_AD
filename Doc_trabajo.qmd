...
matriz_dispersion <- spotify2 %>% 
  select(19:23) %>% 
  ggpairs(aes(color = spotify2$mode, binwidth = 10))

matriz_dispersion
```

Aquí vemos una correlación positiva entre las variables `danceability_%` y `valence_%`, así como entre `energy_%` y `valence_%`. También vemos una correlación negativa entre `energy_%` y `acousticness_%`, que es el par de variables con la correlación más fuerte. También, podemos observar que si nos restringimos a las canciones en modo mayor, presentan una correlación más fuerte.
...
matriz_dispersion_100 <- top100canciones %>% 
  select(19:23) %>% 
  ggpairs(aes(color = top100canciones$mode, bins = 30))
