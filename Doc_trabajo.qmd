...
1.  Características comunes entre las canciones.
  ...
  1.1 Considerando las 100 canciones más reproducidas.
...
Podemos observar en la matriz que tenemos variables que están correlacionadas, sobretodo positivamente. El estudio de estas correlaciones se estudiarán con más detalle en los siguientes apartados.
...
spotify2 %>%
  ...
  na.omit(.) %>% 
  cor(.) %>% 
  corPlot(., cex = 0.5, xsrt = 30, main = "Matriz de correlación reducida")
```
...
matriz_dispersion <- spotify2 %>% 
  select(19:23) %>% 
  ggpairs(aes(color = spotify2$mode), binwidth = 30, title = "Matriz de dispersión(Original)") +
...
```
 X e Y son  `energy%` y `valence%`  respectivamente:
$$\begin{cases}
H_{0}: &  \rho_{X,Y}=0\\
H_{1}: & \rho_{X,Y}>0
\end{cases}$$
...
```
 X e Y son `energy%` y `acoustic%`  respectivamente:
$$\begin{cases}
H_{0}: &  \rho_{X,Y}=0\\
H_{1}: & \rho_{X,Y}<0
\end{cases}$$
...
```
El p-valor $< 0.00000000000000022$ nos da evidencia estadísticamente significativa de que rechazamos las hipotesis nulas y aceptamos las alternativas. Los últimos valores, los $0.4084514$, $0.357612$ y $-0.5773442$ respectivos de cada contraste bajo el cor, son las correlaciones de Pearson de los dos vectores de pesos, que como vemos coinciden con las del gráfico.
...
matriz_dispersion_100 <- top100canciones %>% 
  select(19:23) %>% 
  ggpairs(aes(color = top100canciones$mode), binwidth = 30, alpha = 0.75, title = "Matriz de dispersión(Top 100)")+
  theme(plot.title=element_text(size=20, face='bold', color='black'), plot.subtitle = element_text(size=20, face='bold'))
...
Vemos que el único artista que ha utilizado más la escala menor es "The Weeknd". Por otro lado, destaca que ninguna canción de los artistas presenta la tonalidad $C$ (Do) en modo Mayor, que es la escala más conocida, aunque también básica en cuanto a novedad. Veamos ahora cual es la tonalidad más usada por artista:
...
Para ello vamos a usar la función "ggparis()" que nos da un estudio muy detallado de las relaciones de las variables de reproducción de las canciones con el número de playlists (listas de reproduccion) en las que la canción está metida en las diferentes plataformas. 
...
matriz_dispersionn <- spotify2 %>% 
  ...
  ggpairs(aes(color = spotify2$collaboration))
...
```
Efectivamente encontramos un p-valor $<0.00000000000000022$ que es muy pequeño y nos permite rechazar la hipotesis nula de que son iguales a 0 y aceptar la altenrativa de que la correlación es positiva, en particular de $0.7082772 $.
...
(RESUMEN DE LAS CONCLUSIONES POR APARTADOS)
...
Para terminar, si eres un novato en la música y buscas una manera de que tu canción sea un exito, haz una colaboración con Bad Bunny o Taylor Swift.
